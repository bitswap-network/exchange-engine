...
func ProcessLimitOrder(side Side, orderID string, quantity, price decimal.Decimal) (quantityLeft decimal.Decimal, totalPrice decimal.Decimal, err error) {
	...
	if quantityToTrade.Sign() > 0 {
		...
		o, _ := NewOrder(orderID, side, quantityToTrade, price, time.Now().UTC(), false)
...
func PartialOrder(orderID string, quantityProcessed decimal.Decimal, totalPrice float64) *Order {
	...
	order := e.Value.(*Order)
	partialOrder, err := NewOrder(orderID, order.Side(), order.Quantity().Sub(quantityProcessed), order.Price(), time.Now(), false)
	if err != nil {
